...
# Main script to solve Naked Triples in a Sudoku grid
class_name NakedTriplesSolver

var grid : Array

func _init(puzzle_grid : Array):
	grid = puzzle_grid
...
func solve():
	...
	for i in range(9):
		updated = check_naked_triples_unit(i, true) or updated  # Check rows
		updated = check_naked_triples_unit(i, false) or updated  # Check columns
		updated = check_naked_triples_box(i) or updated  # Check boxes

	return [grid, updated]

# Helper function to check and resolve Naked Triples in a row or column
func check_naked_triples_unit(index: int, is_row: bool) -> bool:
	var updated = false
...
func check_naked_triples_box(box: int) -> bool:
	var updated = false
